...
{
    public class ViewLSPRMPriceRepo : RepositoryBase<vSSP_INF_LSP_RM_PRICE>, IViewLSPRMPriceRepo, IDownloadAccountReportRepo
    {
        #region Inject

        public ViewLSPRMPriceRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_INF_LSP_RM_PRICEs.AsQueryable();
            //if (startdate != null)
            //{
            ...
            //{
            //    query = query.Where(p => planningGroup.Contains(p.PLANNING_GROUP));
            //}
