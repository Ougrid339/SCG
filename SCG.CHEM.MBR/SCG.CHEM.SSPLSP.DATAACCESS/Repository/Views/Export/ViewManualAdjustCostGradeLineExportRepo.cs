...
{
    public class ViewManualAdjustCostGradeLineExportRepo : RepositoryBase<vSSP_MST_MANUAL_ADJUST_COST_GRADELINE_EXPORT>, IViewManualAdjustCostGradeLineExportRepo, IDownloadRepo
    {
        #region Inject

        public ViewManualAdjustCostGradeLineExportRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_MANUAL_ADJUST_COST_GRADELINE_EXPORTs.AsQueryable();
            if (startdate != null)
            {
