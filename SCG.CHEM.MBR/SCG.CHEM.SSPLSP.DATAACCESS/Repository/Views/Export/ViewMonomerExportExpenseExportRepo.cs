...
{
    public class ViewMonomerExportExpenseExportRepo : RepositoryBase<vSSP_MST_MONOMER_EXPORT_EXPENSE_EXPORT>, IViewMonomerExportExpenseExportRepo, IDownloadRepo
    {
        #region Inject

        public ViewMonomerExportExpenseExportRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_MONOMER_EXPORT_EXPENSE_EXPORTs.AsQueryable();
            if (startdate != null)
            {
