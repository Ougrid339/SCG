...
{
    public class ViewRawMatGapExportRepo : RepositoryBase<vSSP_MST_RAW_MAT_GAP_EXPORT>, IViewRawMatGapExportRepo, IDownloadRepo
    {
        #region Inject

        public ViewRawMatGapExportRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }

        #endregion Inject

        public List<vSSP_MST_RAW_MAT_GAP_EXPORT> GetByPlanTypeAndDate(List<string> planTypeList, DateTime startdate)
        {
            var result = _context.vSSP_MST_RAW_MAT_GAP_EXPORTs.Where(s => planTypeList.Contains(s.PlanType) &&
            startdate <= s.FirstDate).ToList();
            return result;
        }

        public List<vSSP_MST_RAW_MAT_GAP_EXPORT> GetByPlanType(List<string> planTypeList)
        {
            var result = _context.vSSP_MST_RAW_MAT_GAP_EXPORTs.Where(s => planTypeList.Contains(s.PlanType)).ToList();
            return result;
        }

        public List<Object> DownloadMaster(List<string> planTypeList = null, string startdate = null, string cycle = null)
        {
            var query = _context.vSSP_MST_RAW_MAT_GAP_EXPORTs.AsQueryable();
            if (startdate != null)
            {
