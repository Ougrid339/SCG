...
{
    public class ViewExchangeRateExportRepo : RepositoryBase<vSSP_MST_EXCHANGE_RATE_EXPORT>, IViewExchangeRateExportRepo, IDownloadRepo
    {
        #region Inject

        public ViewExchangeRateExportRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_EXCHANGE_RATE_EXPORTs.AsQueryable();
            if (startdate != null)
            {
        ...
        public List<vSSP_MST_EXCHANGE_RATE_EXPORT> GetByFirstDate(List<DateTime> firstDate) {
            var data = _readContext.vSSP_MST_EXCHANGE_RATE_EXPORTs.Where(w => w.FirstDate.HasValue && firstDate.Contains(w.FirstDate.Value)).ToList();
            return data;
        }
    }
}