...
{
    public class ViewMappingWaxGroupByGradeExportRepo : RepositoryBase<vSSP_MST_MAPPING_WAX_GROUP_BY_GRADE_EXPORT>, IViewMappingWaxGroupByGradeExportRepo, IDownloadRepo
    {
        #region Inject

        public ViewMappingWaxGroupByGradeExportRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_MAPPING_WAX_GROUP_BY_GRADE_EXPORTs.AsQueryable();
            //if (startdate != null)
            //{
