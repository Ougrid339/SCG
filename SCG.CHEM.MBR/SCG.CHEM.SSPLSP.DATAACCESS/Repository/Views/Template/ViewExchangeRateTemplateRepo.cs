...
{
    internal class ViewExchangeRateTemplateRepo : RepositoryBase<vSSP_MST_EXCHANGE_RATE_TEMPLATE>, IViewExchangeRateTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewExchangeRateTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_EXCHANGE_RATE_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
