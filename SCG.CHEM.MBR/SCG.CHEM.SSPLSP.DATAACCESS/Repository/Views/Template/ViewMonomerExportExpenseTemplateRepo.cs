...
{
    public class ViewMonomerExportExpenseTemplateRepo : RepositoryBase<vSSP_MST_MONOMER_EXPORT_EXPENSE_TEMPLATE>, IViewMonomerExportExpenseTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewMonomerExportExpenseTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_MONOMER_EXPORT_EXPENSE_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
