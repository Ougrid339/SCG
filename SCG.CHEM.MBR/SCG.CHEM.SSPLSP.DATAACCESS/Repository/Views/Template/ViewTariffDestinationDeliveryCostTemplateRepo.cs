...
{
    public class ViewTariffDestinationDeliveryCostTemplateRepo : RepositoryBase<vSSP_MST_TARIFF_DESTINATION_DELIVERY_COST_TEMPLATE>, IViewTariffDestinationDeliveryCostTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewTariffDestinationDeliveryCostTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_TARIFF_DESTINATION_DELIVERY_COST_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
