...
{
    public class ViewManualAdjustCostGradeLineTemplateRepo : RepositoryBase<vSSP_MST_MANUAL_ADJUST_COST_GRADELINE_TEMPLATE>, IViewManualAdjustCostGradeLineTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewManualAdjustCostGradeLineTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_MANUAL_ADJUST_COST_GRADELINE_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
