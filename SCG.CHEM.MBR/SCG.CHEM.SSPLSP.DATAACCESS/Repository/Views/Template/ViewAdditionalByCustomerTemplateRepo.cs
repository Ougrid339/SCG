...
{
    public class ViewAdditionalByCustomerTemplateRepo : RepositoryBase<vSSP_MST_ADDITIONAL_BY_CUSTOMER_TEMPLATE>, IViewAdditionalByCustomerTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewAdditionalByCustomerTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_ADDITIONAL_BY_CUSTOMER_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
