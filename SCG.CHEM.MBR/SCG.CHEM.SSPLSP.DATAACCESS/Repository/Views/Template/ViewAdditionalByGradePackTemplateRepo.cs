...
{
    public class ViewAdditionalByGradePackTemplateRepo : RepositoryBase<vSSP_MST_ADDITIONAL_BY_GRADEPACK_TEMPLATE>, IViewAdditionalByGradePackTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewAdditionalByGradePackTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_ADDITIONAL_BY_GRADEPACK_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
