...
{
    public class ViewWaxViscosityPercentTemplateRepo : RepositoryBase<vSSP_MST_WAX_VISCOSITY_PERCENT_TEMPLATE>, IViewWaxViscosityPercentTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewWaxViscosityPercentTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_WAX_VISCOSITY_PERCENT_TEMPLATEs.AsQueryable();
            if (startdate != null)
            {
