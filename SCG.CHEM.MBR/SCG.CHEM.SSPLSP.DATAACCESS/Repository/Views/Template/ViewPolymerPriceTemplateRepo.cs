...
{
    public class ViewPolymerPriceTemplateRepo : RepositoryBase<vSSP_MST_POLYMER_PRICE_TEMPLATE>, IViewPolymerPriceTemplateRepo, IDownloadRepo
    {
        #region Inject

        public ViewPolymerPriceTemplateRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }
        ...
        {
            var query = _context.vSSP_MST_POLYMER_PRICE_TEMPLATEs.AsQueryable();
            //if (startdate != null)
            //{
            //    query = query.Where(d => startdate == d.StartMonth);
            //}
