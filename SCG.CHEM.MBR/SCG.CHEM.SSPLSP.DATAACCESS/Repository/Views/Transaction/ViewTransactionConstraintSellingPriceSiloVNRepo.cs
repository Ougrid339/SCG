...
{
    public class ViewTransactionConstraintSellingPriceSiloVNRepo : RepositoryBase<vSSP_TRN_CONSTRAINT_SELLING_PRICE_SILO_VN>, IViewTransactionConstraintSellingPriceSiloVNRepo
    {
        #region Inject

        public ViewTransactionConstraintSellingPriceSiloVNRepo(EntitiesContext context, EntitiesReadContext readConext) : base(context, readConext)
        {
        }

        #endregion Inject

        //public List<MonthModel> GetConstraintsSellingPriceSiloVN(string planType, string inputM1)
        //{
        //    List<MonthModel> result = new List<MonthModel>();
        //    result = _context.vSSP_TRN_CONSTRAINT_SELLING_PRICE_SILO_VNs.Where(w => w.PlanType == planType && w.InputM1 == inputM1
        ...
        //       Name = "PriceVN-" + s.MonthIndex,
        //       Value = s.SellingPriceSiloVN ?? 0,
