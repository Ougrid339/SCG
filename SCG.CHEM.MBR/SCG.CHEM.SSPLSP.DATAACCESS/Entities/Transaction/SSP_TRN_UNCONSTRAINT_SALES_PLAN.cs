...
{
    [Table("SSP_TRN_UnconstraintSalesPlan")]
    public class SSP_TRN_UNCONSTRAINT_SALES_PLAN : BaseContext
    {
        [Key]
        [StringLength(50)]
        ...
        public bool IsMarkDeletedByDMO { get; set; }

        [Column(TypeName = "decimal(15,5)")]
        public decimal? MarkDeleteQty { get; set; }

        [Column(TypeName = "decimal(15,5)")]
        public decimal? MarkDeletePrice { get; set; }

        public SSP_TRN_UNCONSTRAINT_SALES_PLAN()
        { }

        public SSP_TRN_UNCONSTRAINT_SALES_PLAN(string planType, string versionName, int scenarioId, string scenarioDesc, string customerCode, string customerName, string companyCode, string channel, string matPrefix, string grade, string package, string matCodeMst, string matCodeTrn, string matGroup, string product, string productSub, int unitId, string unit, int newProductId, string newProductDesc, string salesGroupCode, string salesGroupName, string planningGroup, string region, string hVASegmentCode, string refGrade, string projectID, string reqProductionSite, string subRegion, string salesDistrict, int priceTypeId, int priceUnitId, string inputM1, string monthIndex, string monthNo, decimal? qty, decimal? price, string moveType, bool autoGenFlag, bool isMarkDeletedByDMO, int versionNo)
        {
            PlanType = planType;
            ...
            IsMarkDeletedByDMO = isMarkDeletedByDMO;
            VersionNo = versionNo;
        ...
        public void SetMarkDeleteByDMO(bool isDelete)
        {
            if (this.IsMarkDeletedByDMO != isDelete)
            {
                // Set Qty or Use Backup
                if (isDelete)
                {
                    this.MarkDeleteQty = Qty;
                    this.MarkDeletePrice = Price;
                    this.Qty = 0;
                    this.Price = 0;
                }
                else
                {
                    this.Qty = MarkDeleteQty;
                    this.Price = MarkDeletePrice;
                    this.MarkDeleteQty = 0;
                    this.MarkDeletePrice = 0;
                }

                this.IsMarkDeletedByDMO = isDelete;
