...
{
    internal class BeginningInventoryRepo : RepositoryBase<MBR_TRN_BEGINING_INVENTORY>, IBeginningInventoryRepo
    {
        public BeginningInventoryRepo(EntitiesMBRContext context, EntitiesMBRReadContext readConext) : base(context, readConext)
        {
        }

        public List<MBR_TRN_BEGINING_INVENTORY> FindByCriterias(string scenario, string @case, string cycle)
        {
            return _context.MBR_TRN_BEGINING_INVENTORYs.Where(w => w.PlanType.ToLower() == scenario.ToLower() && w.Case.ToLower() == @case.ToLower() && w.Cycle.ToLower() == cycle.ToLower()).ToList();
        }

        public List<MBR_TRN_BEGINING_INVENTORY> FindByCriterias(string scenario, string @case, string cycle, List<string> companys)
        {
            //return _context.MBR_TRN_BEGINING_INVENTORYs.Where(w => w.PlanType.ToLower() == scenario.ToLower() && w.Case.ToLower() == @case.ToLower() && w.Cycle.ToLower() == cycle.ToLower() && companys.Select(s => s.ToLower()).Contains(w.Company)).ToList();
            var resultValue = _context.MBR_TRN_BEGINING_INVENTORYs.Where(w => w.PlanType.ToLower() == scenario.ToLower() && w.Case.ToLower() == @case.ToLower() && w.Cycle.ToLower() == cycle.ToLower() /*&& companys.Select(s => s.ToLower()).Contains(w.Company)*/).ToList();
            if (companys != null && companys.Count > 0) { resultValue = resultValue.Where(o => companys.Contains(o.Company)).ToList(); }
            return resultValue;
        }
        ...
        public List<OptienceMergeScenario> GetMergeScenario()
        {
            ...
            var typeName = _readContext.MBR_MST_MASTER_EXCELs.FirstOrDefault(s => s.ExcelId == MASTER_EXCEL_TYPE.BEGINNING_INVENTORY)?.MasterName;
            Dictionary<string, List<string>> enumCycle = GetDictionaryCycle(scenarios);
            var data = _context.MBR_TRN_BEGINING_INVENTORYs
                 .Select(s => new OptienceMergeScenario()
                 {
                     TypeId = MASTER_EXCEL_TYPE.BEGINNING_INVENTORY,
                     TypeName = typeName,
                     Case = s.Case,
                     Cycle = s.Cycle,
                     Scenario = s.PlanType
