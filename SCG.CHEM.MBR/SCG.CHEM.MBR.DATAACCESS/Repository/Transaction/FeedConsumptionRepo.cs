...
{
    internal class FeedConsumptionRepo : RepositoryBase<MBR_TRN_FEED_CONSUMPTION>, IFeedConsumptionRepo
    {
        public FeedConsumptionRepo(EntitiesMBRContext context, EntitiesMBRReadContext readConext) : base(context, readConext)
        {
        }

        public List<MBR_TRN_FEED_CONSUMPTION> FindByCriterias(string scenario, string @case, string cycle)
        {
            return _context.MBR_TRN_FEED_CONSUMPTIONs.Where(w => w.PlanType.ToLower() == scenario.ToLower() && w.Case.ToLower() == @case.ToLower() && w.Cycle.ToLower() == cycle.ToLower()).ToList();
        }

        public List<MBR_TRN_FEED_CONSUMPTION> FindByCriterias(string scenario, string @case, string cycle, List<string> companys)
        {
            return _context.MBR_TRN_FEED_CONSUMPTIONs.Where(w => w.PlanType.ToLower() == scenario.ToLower() && w.Case.ToLower() == @case.ToLower() && w.Cycle.ToLower() == cycle.ToLower() && companys.Select(s => s.ToLower()).Contains(w.Company)).ToList();
        }
        ...
        public List<OptienceMergeScenario> GetMergeScenario()
        {
            ...
            var typeName = _readContext.MBR_MST_MASTER_EXCELs.FirstOrDefault(s => s.ExcelId == MASTER_EXCEL_TYPE.FEED_CONSUMPTION)?.MasterName;
            var data = _context.MBR_TRN_FEED_CONSUMPTIONs
                 .Select(s => new OptienceMergeScenario()
                 {
                     TypeId = MASTER_EXCEL_TYPE.FEED_CONSUMPTION,
                     TypeName = typeName,
                     Case = s.Case,
                     Cycle = s.Cycle,
                     Scenario = s.PlanType
