...
namespace SCG.CHEM.MBR.TRANSACTION.API.BusinessLogic.Logging
{
    public sealed class LogService : ILogService
    {
        ...
        public long CreateLogMove(string servicePath, string inboundMessage)
        {
            long logId = 0;
            ...
            LOG_SEND_DWH data = new LOG_SEND_DWH("CallMove", servicePath, inboundTime, outboundTime, inboundMessage, outboundMessage, planType, cycle, "", "");

            _unit.LogSendDWHRepo.Add(data);
            _unit.SaveTransaction();
        ...
        public long CreateLogMove(string servicePath, string inboundMessage, int? type, string planType, string cycle, string caseName)
        {
            long logId = 0;
            ...
            var runStatus = JsonConvert.DeserializeObject<RequestDataFactoryRunIdStatus>(inboundMessage)?.status;

            LOG_SEND_DWH data = new LOG_SEND_DWH(runStatus, servicePath, inboundTime, outboundTime, inboundMessage, outboundMessage, planType, cycle, "", "");
        ...
        public long UpdateLogMove(long id, string servicePath)
        {
            long logId = 0;
            ...
            LOG_SEND_DWH data = _unit.LogSendDWHRepo.FindbyId(id);
            data.UpdateLog(servicePath);
        ...
        public void UpdateLogMoveOnError(long id, string errorMessage)
        {
            LOG_SEND_DWH data = _unit.LogSendDWHRepo.FindbyId(id);
